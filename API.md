...
The `:task_id` is the local agent task job id returned from `POST /api/agents/create-task`.
...
This endpoint returns the exact same response body as `GET /api/files/index-task/:id/files`.
...
### Resolve a branch or tag, then index its files

See [`INDEX_FILES_TASK_API.md`](./INDEX_FILES_TASK_API.md) for the full file
indexing reference; the high-level flow is:
...
3. Create file index tasks with `POST /api/files/index-task` for either or both commits if you want indexed file metadata from this service.
