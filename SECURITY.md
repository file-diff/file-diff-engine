...
| Medium | Viewer credentials can trigger expensive repository-processing work. | `src/routes/files/indexTaskRoutes.ts` protects `POST /api/files/index-task` with `requireViewerBearerToken`, but the route creates database state and enqueues repository-processing jobs. | Consider a separate write-scoped token, stricter per-token quotas, and stronger rate limiting around job creation. |
